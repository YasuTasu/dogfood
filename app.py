...
def load_aafco_standards():
    ...
    if not os.path.exists(aafco_path):
        ...
        return {}
    df = pd.read_excel(aafco_path)
    standards = {}
    for index, row in df.iterrows():
        nutrient_key = row["nutrient"]
        minimum_value = float(row["minimum"] if row["minimum"] != 'Tr' else 0)
        standards[nutrient_key] = minimum_value
    return standards
...
def calculate():
    ...
    total_grams = sum(g for _, g in selected_list)
